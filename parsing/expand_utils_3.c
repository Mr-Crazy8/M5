...
/*   Updated: 2025/06/10 16:16:24 by anel-men         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// // int is_var_append_with_quotes(char *arg)
// // {
// //     if (!arg)
// //         return 0;
        
// //     // Check for += in the string, accounting for quotes
// //     char *pos = arg;
// //     char *plus_pos = NULL;
// //     int in_quotes = 0;
// //     char quote_char = 0;
...
//     // First check: If this is an export command and we're looking at an argument (not the command itself)
//     if (current->cmd && strcmp(current->cmd, "export") == 0 && *i > 0)
//     {
//         // Check for += pattern in original arg (before quote removal)
//         if (current->args_befor_quotes_remover && *i < count && 
//             current->args_befor_quotes_remover[*i])
//         {
//             char *orig_arg = current->args_befor_quotes_remover[*i];
            
//             // Check if there are quotes and a += pattern
//             if (is_var_append_with_quotes(orig_arg) && 
//                 (strchr(orig_arg, '"') || strchr(orig_arg, '\'')))
//             {
//                 // Variable has both quotes and += operator - force split
//                 force_split = 1;
//                 should_split = 1;
//             }
//             // For a plain += without quotes or $ prefix, don't split
//             else if (strstr(orig_arg, "+=") && 
//                      !strchr(orig_arg, '"') && 
//                      !strchr(orig_arg, '\'') && 
//                      orig_arg[0] != '$')
//             {
//                 // Skip regular append assignments without quotes
//                 return;
//             }
//         }
//     }

//     // Also handle explicit $var+= pattern
//     if (current->args_befor_quotes_remover && *i < count && 
//         current->args_befor_quotes_remover[*i] &&
//         current->args_befor_quotes_remover[*i][0] == '$' &&
//         strstr(current->args_befor_quotes_remover[*i], "+="))
//     {
//         // Always force split for variable appends with $ prefix
//         force_split = 1;
//         should_split = 1;
//     }
    
//     // Standard checks for other cases that require splitting
